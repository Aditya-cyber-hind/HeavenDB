...
static void pbkdf2(const char *password, const uint8_t *salt, size_t salt_len,
    ...
    if (salt_len + 4 > sizeof(salt_block)) {
        // Refuse to overflow; should never happen with SALT_SIZE = 16
...
static void generate_salt(uint8_t *salt, size_t len) {
    if (!secure_random_bytes(salt, len)) {
        fprintf(stderr, "FATAL: no secure RNG available for salt generation\n");
        abort();
    }
}
...
void auth_generate_random_password(char *output, int length) {
    ...
    uint8_t *rnd = (uint8_t*)malloc(length);
    if (!rnd) {
        output[0] = '\0';
        return;
    }
    if (!secure_random_bytes(rnd, length)) {
        free(rnd);
        fprintf(stderr, "FATAL: no secure RNG available for password generation\n");
        abort();
    }
    ...
    output[1] = 'a' + (rnd[1] % 26);
    output[2] = '0' + (rnd[2] % 10);
    output[3] = charset[52 + (rnd[3] % 10)];
    
    for (int i = 4; i < length - 1; i++) {
        output[i] = charset[rnd[i] % charset_len];
    }
