...
static SRWLOCK sql_global_lock = SRWLOCK_INIT;

static void generate_uuid(char *output) {
    unsigned char bytes[16];
    if (BCryptGenRandom(NULL, bytes, 16, BCRYPT_USE_SYSTEM_PREFERRED_RNG) != 0) {
        strcpy(output, "00000000-0000-4000-8000-000000000000");
        return;
    }
...
static int handle_create_table(TokenList *tokens) {
    ...
    while (i < tokens->count && strcmp(tokens->tokens[i], ")") != 0) {
        ...
        if (strcmp(col_type, "INTEGER") == 0 || strcmp(col_type, "INT") == 0) {
            ...
            type = TYPE_DATE;
        } else if (strcmp(col_type, "TIMESTAMP") == 0 || strcmp(col_type, "DATETIME") == 0) {
            type = TYPE_TIMESTAMP;
...
static int handle_drop_table(TokenList *tokens) {
    ...
    if (!check_permission(table_name, PERM_DELETE)) return -1;
...
static int handle_alter_table(TokenList *tokens) {
    ...
    if (!check_permission(table_name, PERM_UPDATE)) return -1;
...
static int handle_insert(TokenList *tokens) {
    ...
    if (!check_permission(table_name, PERM_INSERT)) return -1;
    ...
    while (i < tokens->count && col_idx < table->column_count) {
        ...
        // Handle NOW() as a single value
        if (strcmp(tokens->tokens[i], "NOW") == 0 &&
            i + 2 < tokens->count &&
            strcmp(tokens->tokens[i+1], "(") == 0 &&
            strcmp(tokens->tokens[i+2], ")") == 0) {
            
            if (table->columns[col_idx].type == TYPE_TIMESTAMP ||
                table->columns[col_idx].type == TYPE_DATE) {
                char *str = (char*)malloc(64);
                time_t t = time(NULL);
                struct tm *tm_info = localtime(&t);
                if (table->columns[col_idx].type == TYPE_TIMESTAMP) {
                    strftime(str, 64, "%Y-%m-%d %H:%M:%S", tm_info);
                } else {
                    strftime(str, 64, "%Y-%m-%d", tm_info);
                }
                values[col_idx] = str;
            } else {
                printf("ERROR: NOW() used for non-date column\n");
                for (int j = 0; j < col_idx; j++) free(values[j]);
                free(values);
                return -1;
            }
        ...
        // Handle CURRENT_DATE() as a single value
        if (strcmp(tokens->tokens[i], "CURRENT_DATE") == 0 &&
            i + 2 < tokens->count &&
            strcmp(tokens->tokens[i+1], "(") == 0 &&
            strcmp(tokens->tokens[i+2], ")") == 0) {
            
            if (table->columns[col_idx].type == TYPE_DATE ||
                table->columns[col_idx].type == TYPE_TIMESTAMP) {
                char *str = (char*)malloc(64);
                time_t t = time(NULL);
                struct tm *tm_info = localtime(&t);
                if (table->columns[col_idx].type == TYPE_DATE) {
                ...
                printf("ERROR: CURRENT_DATE() used for non-date column\n");
                for (int j = 0; j < col_idx; j++) free(values[j]);
                free(values);
                return -1;
            }
...
static int handle_date_function(TokenList *tokens) {
    if (tokens->count < 3) return -1;
    ...
    if (strcmp(func_name, "CURRENT_DATE") == 0) {
        time_t t = time(NULL);
        struct tm *tm_info = localtime(&t);
        char buf[64];
    ...
    // For functions that need a column: YEAR(col), MONTH(col), DAY(col)
    int from_idx = -1;
    for (int i = 1; i < tokens->count; i++) {
        if (strcmp(tokens->tokens[i], "FROM") == 0) {
            from_idx = i;
    ...
    if (from_idx == -1) {
        printf("ERROR: This function requires FROM clause\n");
        return -1;
    }
    ...
    // Handle YEAR(col), MONTH(col), DAY(col)
    if (strcmp(func_name, "YEAR") == 0 ||
        ...
        for (size_t r = 0; r < table->row_count; r++) {
            void **row = (void**)table->rows[r];
            char *date_str = (char*)row[col_idx];
            ...
            if (strcmp(func_name, "YEAR") == 0) {
                char buf[5];
                strncpy(buf, date_str, 4);
                buf[4] = '\0';
                result = atoi(buf);
            } else if (strcmp(func_name, "MONTH") == 0) {
                char buf[3];
                strncpy(buf, date_str + 5, 2);
                buf[2] = '\0';
                result = atoi(buf);
            } else if (strcmp(func_name, "DAY") == 0) {
                char buf[3];
                strncpy(buf, date_str + 8, 2);
                buf[2] = '\0';
                result = atoi(buf);
            }
    ...
    printf("ERROR: Unknown date function '%s'\n", func_name);
    return -1;
}
...
static int sql_execute_inner(const char *sql) {
    ...
    // String, JSON, Date functions (first word check)
    if (strcmp(command, "SELECT") == 0 && list.count > 1) {
        ...
        // Date/Time functions
        if (strcmp(first_word, "NOW") == 0 ||
            strcmp(first_word, "CURRENT_DATE") == 0 ||
            strcmp(first_word, "YEAR") == 0 ||
            strcmp(first_word, "MONTH") == 0 ||
            strcmp(first_word, "DAY") == 0) {
            return handle_date_function(&list);
        }
...
int sql_execute(const char *sql) {
    AcquireSRWLockExclusive(&sql_global_lock);
    int result = sql_execute_inner(sql);
    ReleaseSRWLockExclusive(&sql_global_lock);
    return result;
}